...
NestJS requires CommonJS. The `"module"` setting only controls what the _compiled JavaScript output_ looks like — it does not affect TypeScript syntax. You still write `import`, `export default`, `import type`, etc. throughout the codebase. TypeScript transpiles these into `require()`/`module.exports` calls in the compiled `.js` files, which you never see.
...
NestJS deliberately adopts Angular's file naming conventions (`*.service.ts`, `*.module.ts`, `*.controller.ts`, kebab-case filenames). From the official NestJS docs: _"The architecture is heavily inspired by Angular."_ The Angular icon from your IDE's file icon theme is expected and correct.
