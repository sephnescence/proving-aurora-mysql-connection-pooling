...
const mockRows: ProcesslistRow[] = [
  { ID: 1, USER: 'admin', HOST: 'localhost', DB: 'pooling_poc', COMMAND: 'Query', TIME: 0, STATE: null, INFO: null },
];
...
describe('DbQueryService', () => {
  ...
  it('propagates error thrown by mysql', async () => {
    ...
    await expect(service.processlist('sequential')).rejects.toThrow('connection refused');
  });
});
```
