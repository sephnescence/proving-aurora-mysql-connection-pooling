# Step 00010 — Create DbQueryService

## Context
The core service that reads `DB_CONNECTION_MODE` from the environment and executes the processlist query. In `persistent` mode a single connection is created on module init and reused. In `per-request` mode a new connection is created and destroyed for every call.

## Action
Create the file `packages/db-query/src/db-query.service.ts` with the following exact content:
...
## Expected result
`packages/db-query/src/db-query.service.ts` exists.
