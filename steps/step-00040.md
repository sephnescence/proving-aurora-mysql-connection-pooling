# Step 00040 — Update integration tests to expect 200

## Context

Now that Docker Compose is configured (step-00038) and `.env` is in place (step-00039), both apps have a real database. Update the integration tests to assert the correct happy-path behaviour: HTTP 200 with a non-empty array of processlist rows.
