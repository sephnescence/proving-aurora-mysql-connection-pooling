...
Now that Docker Compose is configured (step-00033) and `.env` is in place (step-00032), both apps have a real database. Update the integration tests to assert the correct happy-path behaviour: HTTP 200 with a non-empty array of processlist rows.
